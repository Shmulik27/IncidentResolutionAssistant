...
REQUESTS_TOTAL = Counter(
    "kb_search_requests_total", "Total requests to knowledge base search", ["endpoint"]
)
ERRORS_TOTAL = Counter(
    "kb_search_errors_total", "Total errors in knowledge base search", ["endpoint"]
)
SEARCHES_TOTAL = Counter(
    "kb_search_searches_total", "Total searches made", ["endpoint"]
)
...
INCIDENTS = [
    {
        "id": 1,
        "text": "Service X crashed due to out of memory error",
        "resolution": "Restarted service X and increased memory limit",
    },
    {
        "id": 2,
        "text": "Database connection timeout from service Y",
        "resolution": "Checked network and restarted service Y",
    },
    {
        "id": 3,
        "text": "Disk full on /dev/sda1",
        "resolution": "Cleaned up old logs and freed space",
    },
    {
        "id": 4,
        "text": "Permission denied error when accessing /etc/passwd",
        "resolution": "Fixed file permissions",
    },
]
...
def search_incidents(request: SearchRequest) -> list[SearchResult]:
    ...
    try:
        ...
        if not request.query:
            ...
            return []
        query_list: list[str] = [str(request.query)]
        query_emb = MODEL.encode(query_list, convert_to_numpy=True)  # type: ignore
